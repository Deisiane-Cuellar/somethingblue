	...
	<section>
		<nav>
			<ul id="menu">
				...
				<li><a href="emprestados.php" class="active">Itens emprestados</a></li>
		...
		<table border="1">
				...
				$sql = "SELECT p.id AS idpegaremprestado, p.dtfinal, e.*,  u.* FROM pegaremprestado AS p 
					left join empreste AS E ON E.id=p.id_empreste 
					left join usuarios AS U ON U.id=p.id_usuarios
						WHERE NOW() < dtfinal AND entregue=0 ";
				...
				if ($res) {
					...
					while ($row = mysqli_fetch_assoc($res)) {
						...
						echo "<tr>
								...
								<td>" . $row['cor'] . "</td>
								<td>" . $row['dtfinal'] . "</td>
								<td><a href='devolver.php?p=e&id=" . $row['idpegaremprestado'] . "'>Devolver</a></td>
